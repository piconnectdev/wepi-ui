...
import { createPayment } from "../../../shared/pisdk";
...
> {
  ...
  async toggleBlockPerson(recipientId: string, block: boolean) {
  ...
  async handlePiBlockchainClick(i: Profile) {
    var config;
    const person = i.state.personRes?.person_view.person;
    if (person) {
      config = {
        amount: 0.00001,
  ...
  handleUnblockPerson(personId: string) {
